...
public class DayFour {
    ...
    public long getUniquePassphraseCount() {
        return passphraseWords.stream().filter(this::hasNoDuplicates).count();
    }

    public long getNonAnagramPassphraseCount() {
        return passphraseWords.stream().filter(this::hasNoAnagrams).count();
    }

    private boolean hasNoAnagrams(String[] words) {
        return checkWords(words, this::sortWord);
    }
    ...
    private boolean hasNoDuplicates(String[] words) {
        return checkWords(words, Function.identity());
    }

    private boolean checkWords(String[] words, Function<String, String> wordTransformer) {
        Set<String> wordSet = new HashSet<>();
        for (String word : words) {
            String transformedWord = wordTransformer.apply(word);
            if (wordSet.contains(transformedWord)) {
                return false;
            }
            wordSet.add(transformedWord);
        }
        return true;
    }
}
